...
def optimize_linear(grad, eps, ord=np.inf):
    ...
    if ord == np.inf:
        ...
        square = tf.maximum(avoid_zero_div, tf.reduce_sum(tf.square(grad), reduction_indices=red_ind, keepdims=True))
...
def get_other_classes_batch(nb_classes, current_classes):
    ...
    :param current_classes: given the class label, integer
...
def get_files(src, file_ext=""):
    ...
    if os.path.isdir(src):
        files = [os.path.join(src, name) for name in os.listdir(src) if
                 os.path.isfile(os.path.join(src, name)) and os.path.splitext(name)[1] == file_ext]
    return files
    ...
    def __init__(self, dataX, datay, batch_size, n_epochs=None, n_steps=None, name='train'):
        ...
        if self.dataX.shape[0] % self.batch_size > 0:
            ...
            if (self.dataX.shape[0] > self.batch_size) and \
                ...
                np.random.seed(0)
                rdm_idx = np.random.choice(self.dataX.shape[0], self.batch_size - self.dataX.shape[0] % self.batch_size,
                                           replace=False)
...
def get_batch_size(integer, max_number_accepted=50):
    ...
    if isinstance(integer, int):
        ...
        raise TypeError("Incorrect input type, Int is accepted.")
